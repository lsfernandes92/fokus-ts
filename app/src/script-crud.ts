...
const labelActiveTaskDescription = document.querySelector<HTMLParagraphElement>(".app__section-active-task-description")
const btAddTask = document.querySelector<HTMLButtonElement>(".app__button--add-task")
const btRemoveAllTasks = document.querySelector<HTMLButtonElement>("#btn-remover-todas")
const btRemoveFinishedTasks = document.querySelector<HTMLButtonElement>("#btn-remover-concluidas")
const formAddTask = document.querySelector<HTMLFormElement>(".app__form-add-task")
const formLabel = document.querySelector<HTMLLabelElement>(".app__form-label")
const formTextArea = document.querySelector<HTMLTextAreaElement>(".app__form-textarea")
const formBtSave = document.querySelector<HTMLButtonElement>(".app__form-footer__button--confirm")
const formBtCancel = document.querySelector<HTMLButtonElement>(".app__form-footer__button--cancel")
const formBtDelete = document.querySelector<HTMLButtonElement>(".app__form-footer__button--delete")

formBtCancel.addEventListener("click", () : void => {
  formAddTask.classList.add("hidden")
  formTextArea.value = ''
})

formBtDelete.addEventListener("click", () : void => {
  initialState = deleteTask(initialState, initialState.selectedTask)
  formAddTask.classList.add("hidden")
  updateUI()
})

btAddTask.addEventListener("click", () : void => {
  formAddTask.classList.remove("hidden")
  formTextArea.value = ''
  updateFormTo("add")
})

btRemoveAllTasks.addEventListener("click", () : void => {
  initialState = deleteAllTasks(initialState)
  updateUI()
})

btRemoveFinishedTasks.addEventListener("click", () : void => {
  initialState = deleteFinishedTasks(initialState)
  updateUI()
})

const updateUI = () : void => {
  formAddTask.onsubmit = (event) : void => {
  ...
  initialState.tasks.forEach(task => {
    ...
    btEdit.addEventListener("click", () : void => {
      formAddTask.classList.remove("hidden")
      updateFormTo("edit")
